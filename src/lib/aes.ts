...
class Aes {
    ...
    static cipher(input:any, w:any) {
        ...
        let state:any = [ [], [], [], [] ];  // initialise 4×Nb byte-array 'state' with input [§3.4]
     ...
     * @private
     */
    static addRoundKey(state:any, w:any, rnd:any, Nb:any) {
        for (let r=0; r<4; r++) {
...
class AesCtr extends Aes {
     ...
     */
    static encrypt(plaintext:any, password:any, nBits:any) {
        ...
        const plaintextBytes = plaintext.split('').map((ch:any) => ch.charCodeAt(0));
     ...
     * @private
     */
    static nistEncryption(plaintext:any, key:any, counterBlock:any) {
     ...
     */
    static decrypt(ciphertext:any, password:any, nBits:128|192|256) {
        if (![ 128, 192, 256 ].includes(nBits)) throw new Error('Key size is not 128 / 192 / 256');
    ...
    static mule(ciphertext:any, password:any, nBits:any){
     ...
     * @private
     */
    static nistDecryption(ciphertext:any, key:any, counterBlock:any) {
        const blockSize = 16; // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
    ...
    static base64Decode(str:any) {
        ...
        if (typeof Buffer != 'undefined') return Buffer.from(str, 'base64').toString('binary'); // Node.js
